...
=begin
We get an exception when we run this code. 
problem is in exists? we call a block and do e == employee. Employee#== calls serial_number
that is a private method. So we can call it on self, but not another object. We can fix 
this by making the attr_reader for serial_number protected instead of private. 
=end