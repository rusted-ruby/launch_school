...
=begin
what's wrong here? Thought it was that we didn't mix in the module correctly. That's right,
but the part I missed was that we do an accitental method override. We don't get a noMethodError
when we run this originally. We get an error in the method that we call, which is Object#send.
So Ruby finds a method to call, its just the wrong one. 
=end
