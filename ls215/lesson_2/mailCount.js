/*//build a function to parse a string of email data. take the data, parse it, then return two
//basic stats about the email: 
  number of email messages in the string
  date range of email messages.

an email message is a string that has the following features
  multiple email messages separated by a delimiter of ##||##
  five parts in the message. Each part is separated with #/#
  the five parts are 
    sender
    subject
    date
...
function mailCount(emailData) {
  let regex = /##\|\|##/;
  let emailArr = emailData.split(regex)
  //console.log(emailArr.length);
  emailArr.forEach((email) => {
    let emailRegex = /#\/#/;
    let oneEmail = email.split(emailRegex)
