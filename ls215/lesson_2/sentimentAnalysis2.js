//now do the same thing with a regex.
...
function sentiment(text) {
  let regex = /\W+/
  let arr = text.toLowerCase().split(regex);
  let posWords = [];
  let negWords = [];
  arr.forEach((word) => {
...
what did LS do? they called match on the entire text for the positive and negative regex!
that just returns every word from the text that matches the regex! no loops are needed!
That's pretty cool.
*/
